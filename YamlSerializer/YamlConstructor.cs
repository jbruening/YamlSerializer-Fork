    ...
    internal class YamlConstructor
    {
        ...
        object ScalarToObject(YamlScalar node, Type type)
        {
            ...
            if(type.IsArray)
            {
                ...
                var declaringType = type.GetElementType();
                var items = node.Value.Split(new[] {','}, StringSplitOptions.RemoveEmptyEntries);
                var result = Array.CreateInstance(declaringType, items.Length);
                for (var i = 0; i < items.Length; i++)
                {
                    var value = Convert.ChangeType(items[i], declaringType);
            ...
            foreach(var entry in map){
                ...
                default:
                    if (!access.ContainsKey(name))
                        continue; // Ignore properties that is not present in the YAML document
